...
from src.gl_encrypted_weight import encrypted_weight_two_linear, failure_payload
...
def flatten(meta: dict) -> dict:
    ...
    return {
        "condition": "gl_encrypted_weight_pca32_two_linear",
        "n_samples": meta.get("n_samples"),
        "ok": meta.get("ok"),
        "semantic_validation_passed": meta.get("semantic_validation_passed"),
        "task_type": meta.get("task_type", "two_linear"),
...
def main() -> None:
    ...
    try:
        meta = encrypted_weight_two_linear(args.n_samples)
    except Exception as exc:
        meta = failure_payload(exc, n_samples=args.n_samples, task_type="two_linear", weight_privacy="encrypted_weight", activation="none")
    base = RESULTS / f"gl_encrypted_weight_pca32_two_linear_n{args.n_samples}"
